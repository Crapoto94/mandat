...
// migrations/016_access_groups.sql pour le contexte. Ne s'applique que sur
// le parcours de connexion LDAP direct (services/ldapAuth.js) : c'est lui
// seul qui fournit de façon fiable le matricule (employeeID) et les
// appartenances de groupe (memberOf) nécessaires pour trancher.
...
/** Autorise ou non un agent authentifié par LDAP direct (les comptes admin
 * locaux sont toujours autorisés, en amont, avant cet appel). Si aucun
 * groupe n'est activé, personne n'est autorisé — verrouillage explicite. */
async function isAgentAuthorized({ employeeId, memberOfDns }) {
  const enabled = await db.all(`SELECT kind, ref_code FROM access_groups WHERE enabled = true`);
  if (!enabled.length) return false;

  for (const g of enabled) {
    if (g.kind === 'niveau' && employeeId) {
      ...
      const row = await db
        ...
        .catch(() => null);
      if (row) return true;
    } else if (g.kind === 'custom_group') {
      const grp = await db.get(`SELECT ad_group_dn FROM hub.custom_groups WHERE id = $1`, [g.ref_code]).catch(() => null);
      if (grp && Array.isArray(memberOfDns) && memberOfDns.includes(grp.ad_group_dn)) return true;
    }
  }
  return false;
}
