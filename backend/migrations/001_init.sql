-- Suivi des engagements du mandat — schéma dédié (nom paramétré via POSTGRES_SCHEMA,
-- cf. backend/db/pg_db.js et scripts/migrate.js qui substituent __SCHEMA__ ci-dessous).
...
CREATE TABLE IF NOT EXISTS __SCHEMA__.etats (
...
INSERT INTO __SCHEMA__.etats (code, libelle, ordre, couleur) VALUES
...
CREATE TABLE IF NOT EXISTS __SCHEMA__.groupes (
...
CREATE TABLE IF NOT EXISTS __SCHEMA__.directions (
  code        VARCHAR(40) PRIMARY KEY,
  libelle     VARCHAR(255)
);

-- Répartition des groupes de travail CODIR (cf. consigne de répartition transmise).
INSERT INTO __SCHEMA__.groupes (code, nom, directions, ordre) VALUES
...
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS __SCHEMA__.admin_users (
  id            SERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS __SCHEMA__.agents_cache (
...
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS __SCHEMA__.engagements (
  id                          SERIAL PRIMARY KEY,
  ...
  groupe_id                   INTEGER REFERENCES __SCHEMA__.groupes(id),  -- NULL = hors groupe (ex: engagement piloté par le Cabinet)
...
CREATE INDEX IF NOT EXISTS idx_engagements_groupe ON __SCHEMA__.engagements(groupe_id);
CREATE INDEX IF NOT EXISTS idx_engagements_etat ON __SCHEMA__.engagements(etat_code);
CREATE INDEX IF NOT EXISTS idx_engagements_axe ON __SCHEMA__.engagements(axe);
...
CREATE TABLE IF NOT EXISTS __SCHEMA__.engagement_history (
  id             SERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_history_engagement ON __SCHEMA__.engagement_history(engagement_id);

-- Fil de commentaires par engagement
CREATE TABLE IF NOT EXISTS __SCHEMA__.comments (
  id             SERIAL PRIMARY KEY,
  engagement_id  INTEGER NOT NULL REFERENCES __SCHEMA__.engagements(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_comments_engagement ON __SCHEMA__.comments(engagement_id);

-- ---------------------------------------------------------------------------
-- Sujets de coordination transversaux (peuvent dépasser un seul engagement)
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS __SCHEMA__.coordination_topics (
  id             SERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS __SCHEMA__.coordination_topic_engagements (
  topic_id       INTEGER NOT NULL REFERENCES __SCHEMA__.coordination_topics(id) ON DELETE CASCADE,
  engagement_id  INTEGER NOT NULL REFERENCES __SCHEMA__.engagements(id) ON DELETE CASCADE,
  PRIMARY KEY (topic_id, engagement_id)
);

-- Trigger simple : maintenir updated_at à jour sur engagements / coordination_topics
CREATE OR REPLACE FUNCTION __SCHEMA__.set_updated_at() RETURNS trigger AS $$
BEGIN
  NEW.updated_at = now();
  RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS trg_engagements_updated_at ON __SCHEMA__.engagements;
CREATE TRIGGER trg_engagements_updated_at BEFORE UPDATE ON __SCHEMA__.engagements
  FOR EACH ROW EXECUTE FUNCTION __SCHEMA__.set_updated_at();

DROP TRIGGER IF EXISTS trg_coordination_updated_at ON __SCHEMA__.coordination_topics;
CREATE TRIGGER trg_coordination_updated_at BEFORE UPDATE ON __SCHEMA__.coordination_topics
  FOR EACH ROW EXECUTE FUNCTION __SCHEMA__.set_updated_at();
