...
router.get('/', requireAuth, async (req, res) => {
  const topics = await db.all(`SELECT * FROM coordination_topics ORDER BY updated_at DESC`);
  const links = await db.all(`SELECT * FROM coordination_topic_engagements`);
...
router.post('/', requireAuth, async (req, res) => {
  ...
  const topic = await db.get(
    `INSERT INTO coordination_topics (titre, description, statut, directions_concernees, created_by)
...
router.patch('/:id', requireAuth, async (req, res) => {
  const { titre, description, statut, directions_concernees, engagement_ids } = req.body || {};
  const topic = await db.get(`SELECT * FROM coordination_topics WHERE id = $1`, [req.params.id]);
  if (!topic) return res.status(404).json({ error: 'Sujet introuvable' });

  const updated = await db.get(
    `UPDATE coordination_topics SET
  ...
  if (engagement_ids) {
    await db.run(`DELETE FROM coordination_topic_engagements WHERE topic_id = $1`, [req.params.id]);
...
router.delete('/:id', requireAuth, async (req, res) => {
  const result = await db.run(`DELETE FROM coordination_topics WHERE id = $1`, [req.params.id]);
...
async function linkEngagements(topicId, engagementIds) {
  ...
  for (const engagementId of engagementIds) {
    await db
      .run(
        `INSERT INTO coordination_topic_engagements (topic_id, engagement_id)
